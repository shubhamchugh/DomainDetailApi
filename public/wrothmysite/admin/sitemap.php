...
<head>
<meta charset="utf-8">
<title>Sitemap - Worth My Site</title>
...
if ($_SERVER['REQUEST_METHOD'] == POST)
{
    ...
    $changefreq =   htmlspecialchars(Trim($_POST['changefreq']));
...
unlink('../sitemap.xml');
$c_date = date('Y-m-d');
$c_sitemap.= '<?xml version="1.0" encoding="UTF-8"?>
        <urlset xmlns="https://www.sitemaps.org/schemas/sitemap/0.9">
<url>
        <loc>'.$protocol.$_SERVER['SERVER_NAME'].'/</loc>
...
$rec_limit = 10;   
$query = "SELECT count(id) FROM domains_data";
...
$sql = "SELECT * FROM domains_data LIMIT $offset, $rec_count ";
...
if($mod_rewrite == "1")
    $server_name = $protocol.$_SERVER['SERVER_NAME']."/".$domain;   
else
    $server_name = $protocol.$_SERVER['SERVER_NAME']."/result.php?domain=".$domain;    
...
$c_sitemap.='
    ...
    file_put_contents("../sitemap.xml",$c_sitemap); 
}
?>                         
                                     <!-- general form elements -->
                            <div class="box box-primary">
                                ...
                                if (isset($_GET['re']))
{
        ...
        <div class="alert alert-success alert-dismissable">
                                        <i class="fa fa-check"></i>
                                        <button aria-hidden="true" data-dismiss="alert" class="close" type="button">x</button>
                                        <b>Alert!</b> Sitemap rebuild successfully
                                    </div>';
                                    }
                                ?>
